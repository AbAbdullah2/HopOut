...
describe(`Test ${endpoint}`, () => {
  ...
  describe('remove a friend request', () => {
  ...
  describe('accept a friend request', () => {
    it('Respond 200 (friend request accepted sucessfully)', async () => {
      ...
      expect(response.body.data.friends[0].user).toBe(requesterId);
      expect(requester.friends[0].user.toString()).toBe(receiverId);

      //acceptor not in requester's sent friends
      expect(requester.sentFriends.length).toBe(0);
  ...
  describe('remove a current friend', () => {
    it('Respond 200 (unfriended a friend sucessfully)', async () => {
      ...
      expect(requester.friends.length).toBe(0);
    });
